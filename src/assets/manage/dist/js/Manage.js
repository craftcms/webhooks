  ...
  var Manager = Garnish.Base.extend({
    ...
    init: function () {
      ...
      $('#content .lightswitch').on('change', function () {
        ...
        Craft.postActionRequest(
          'webhooks/webhooks/save',
          data,
          function (response, textStatus, jqXHR) {
            if (textStatus === 'success' && response.success) {
              Craft.cp.displayNotice(
                enabled
                  ? Craft.t('webhooks', 'Webhook enabled.')
                  : Craft.t('webhooks', 'Webhook disabled.')
              );
            } else {
    ...
    addNewGroup: function () {
      ...
      if (name) {
        ...
        Craft.postActionRequest(
          'webhooks/manage/save-group',
          data,
          $.proxy(function (response, textStatus) {
            if (textStatus === 'success') {
              if (response.success) {
                location.href = Craft.getUrl(
                  'webhooks/group/' + response.group.id
                );
              } else if (response.errors) {
                var errors = this.flattenErrors(response.errors);
                alert(
                  Craft.t('webhooks', 'Could not create the group:') +
                    '\n\n' +
                    errors.join('\n')
                );
              } else {
          ...
          $.proxy(function (response, textStatus) {
            if (textStatus === 'success') {
              if (response.success) {
                ...
                var errors = this.flattenErrors(response.errors);
                alert(
                  Craft.t('webhooks', 'Could not rename the group:') +
                    '\n\n' +
                    errors.join('\n')
                );
              } else {
    ...
    promptForGroupName: function (oldName) {
      return prompt(
        Craft.t('webhooks', 'What do you want to name the group?'),
        oldName
      );
    },
    ...
    deleteSelectedGroup: function () {
      if (
        confirm(
          Craft.t('webhooks', 'Are you sure you want to delete this group?')
        )
      ) {
        var data = {
          id: this.$selectedGroup.data('id'),
